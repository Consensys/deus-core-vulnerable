...
const { setBalance, transfer, swapETH, getBalanceOf} = require("../scripts/helpers/modify_chain.js");
...
describe("DynamicRedeem tests", async () => {
  let dei = '0xde12c7959e1a72bbe8a5f7a1dc8f8eef9ab011b3';
  ...
  before(async () => {
    ...
    await swapETH(deployer, _uniswap_router, BigInt(1e30), [_wrapped_ftm_address, _usdc_address]);
    console.log(await getBalanceOf(collateral, deployer.address));
    await transfer(collateral, deployer.address, pool.address, BigInt(20e6 * 1e6));
    await pool.grantRole(await pool.TRUSTY_ROLE(), deployer.address);
    await pool.addWallet(['0xdDf169Bf228e6D6e701180E2e6f290739663a784', '0xf3fAD54c3b48eE10D676D568A386A4804A9F6814'])
  });
  it("should pass", async () => {
    console.log(await pool.getCirculatingSupply());
    console.log(await pool.getCollateralRatio());
    const deiContract = await hre.ethers.getContractAt('@openzeppelin/contracts/token/ERC20/IERC20.sol:IERC20', dei);
    const deiAmount = BigInt(100e18);
    await deiContract.approve(pool.address, deiAmount);
    await pool.redeemFractionalDEI(deiAmount);
    const usdcBalanceBeforeRedeem = (await getBalanceOf(collateral, deployer.address)).toBigInt();
    const current = await getCurrentTimeStamp();
    await network.provider.send("evm_mine", [current + 35]);
    await pool.collectCollateral();
    console.log((await getBalanceOf(collateral, deployer.address)).toBigInt() - usdcBalanceBeforeRedeem)
  });
});
