  ...
  ReentrancyGuardUpgradeable
{
  ...
  function withdrawTo(uint256 nftIndex, address to) public nonReentrant {
    uint256 nftId = userNfts[msg.sender][nftIndex];
    require(nftUser[nftId] == msg.sender, "Staking: SENDER_IS_NOT_NFT_ONWER");
    require(
  ...
  function withdrawAll(uint256 step, address to) external {
    uint256 userNftsLength = userNftIndex[msg.sender];
    if (userNftsLength > step) {
      userNftsLength = step;
    }
