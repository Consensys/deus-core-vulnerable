...
library DEIPoolLibrary {
    ...
    function calcMintAlgorithmicDEI(uint256 deus_price_usd, uint256 deus_amount_d18) public pure returns (uint256) {
        return deus_amount_d18.mul(deus_price_usd).div(1e6);
    }

    // Must be internal because of the struct
    function calcMintFractionalDEI(MintFF_Params memory params) internal pure returns (uint256, uint256) {
        ...
        {    
            ...
            deus_dollar_value_d18 = params.deus_amount.mul(params.deus_price_usd).div(1e6);
            c_dollar_value_d18 = params.collateral_amount.mul(params.col_price_usd).div(1e6);

        }
        uint calculated_deus_dollar_value_d18 = 
                    (c_dollar_value_d18.mul(1e6).div(params.col_ratio))
                    .sub(c_dollar_value_d18);

        uint calculated_deus_needed = calculated_deus_dollar_value_d18.mul(1e6).div(params.deus_price_usd);

        return (
            c_dollar_value_d18.add(calculated_deus_dollar_value_d18),
            calculated_deus_needed
        );
    }

    function calcRedeem1t1DEI(uint256 col_price_usd, uint256 DEI_amount) public pure returns (uint256) {
        return DEI_amount.mul(1e6).div(col_price_usd);
    }

    // Must be internal because of the struct
    function calcBuyBackDEUS(BuybackDEUS_Params memory params) internal pure returns (uint256) {
        ...
        uint256 deus_dollar_value_d18 = params.DEUS_amount.mul(params.deus_price_usd).div(1e6);
    ...
    function calcRecollateralizeDEIInner(
        ...
        uint256 effective_collateral_ratio = global_collat_value.mul(1e6).div(dei_total_supply); //returns it in 1e6
        uint256 recollat_possible = (global_collateral_ratio.mul(dei_total_supply).sub(dei_total_supply.mul(effective_collateral_ratio))).div(1e6);
