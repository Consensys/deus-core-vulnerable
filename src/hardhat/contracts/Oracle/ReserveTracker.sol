...
contract ReserveTracker is Owned {
    ...
    function setChainlinkDEUSOracle(address _chainlink_deus_oracle) external onlyByOwnerOrGovernance {
        chainlink_deus_oracle = ChainlinkFXSUSDPriceConsumer(_chainlink_deus_oracle);
        chainlink_deus_oracle_decimals = uint256(chainlink_deus_oracle.getDecimals());
    }

    // Get the pair of which to price DEI from
    function setDEIPriceOracle(address _dei_price_oracle_address, address _dei_pair_collateral_address, uint256 _dei_pair_collateral_decimals) public onlyByOwnerOrGovernance {
        dei_price_oracle_address = _dei_price_oracle_address;
        dei_pair_collateral_address = _dei_pair_collateral_address;
        dei_pair_collateral_decimals = _dei_pair_collateral_decimals;
        dei_price_oracle = UniswapPairOracle(dei_price_oracle_address);
        CONSULT_DEI_DEC = 1e6 * (10 ** (uint256(18).sub(dei_pair_collateral_decimals)));
    }

    function setMetapool(address _dei_metapool_address) public onlyByOwnerOrGovernance {
        dei_metapool_address = _dei_metapool_address;
        dei_metapool = IMetaImplementationUSD(_dei_metapool_address);
    }

    // Get the pair of which to price DEUS from (using DEUS-WETH)
    function setDEUSETHOracle(address _deus_weth_oracle_address, address _weth_address) public onlyByOwnerOrGovernance {
        deus_weth_oracle_address = _deus_weth_oracle_address;
        weth_address = _weth_address;
        deus_weth_oracle = UniswapPairOracle(deus_weth_oracle_address);
    }
    ...
    function setETHCollateralOracle(address _weth_collateral_oracle_address, uint _collateral_decimals) public onlyByOwnerOrGovernance {
        ...
        CONSULT_DEUS_DEC = 1e6 * (10 ** (uint256(18).sub(_collateral_decimals)));
    }
